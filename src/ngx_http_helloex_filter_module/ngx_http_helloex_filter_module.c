...
static ngx_int_t ngx_http_helloex_filter_body_filter(ngx_http_request_t *r, ngx_chain_t *in);
static char *ngx_http_helloex_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static ngx_http_helloex_filter_create_conf(ngx_conf_t *cf);
// 过滤模块链表入口
...
static ngx_int_t ngx_http_helloex_filter_header_filter(ngx_http_request_t *r)
{
	...
	// ctx的变量赋值
	ctx->add_prefix = 0;
	// 把ctx设置到ngx_http_helloex_filter_module模块中
